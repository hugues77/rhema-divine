...
<section class="jumbotron text-center">
  <div class="container">
    <h1 class="jumbotron-heading">Musiques Evangéliques</h1>
    <p class="lead text-muted">Bienvenue dans l'espace dédiée aux cantiques évangéliques publié par Rhema divine, Vous pouvez en réchercher via la barre de récherche par titre ou artiste.</p>
    ...
    <form action="" method="POST" class="form-group">
        <?php
          if(isset($_POST['btn_pred'])){
            ...
            if(empty($titre)){
              ...
              $rqt ="SELECT * FROM article WHERE categorie_article='Musique' AND CONCAT(auteur,titre) LIKE '%$titre%' ";
              $req_run = $connexion->query($rqt);
              $reslt = $req_run->fetchAll(PDO::FETCH_OBJ);
              $data = $req_run->rowCount();

              //requete pour les nombre de recherche
              $rqtt ="SELECT COUNT(*) as nb_res FROM article WHERE categorie_article='Musique' AND CONCAT(auteur,titre) LIKE '%$titre%' ";
        ...
        <div class="input-group mb-2"> 
              ...
              <div class="input-group-prepend">
                  <button type="submit" name="btn_pred" class="input-group-text btn btn-outline-primary btn_valider_pred">valider</button>
              </div>
        </div>
    </form>
  </div>
